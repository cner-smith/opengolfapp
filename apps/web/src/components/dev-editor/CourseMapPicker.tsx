...
interface CourseMapPickerProps {
   ...
   *  to already exist to be draggable. Course mode passes the fixed
   *  'course' id; the two-marker hole picker passes whichever of
   *  'tee'/'pin' is currently being placed (HolesEditor's `placing` state),
   *  so a click always targets the marker actively being placed and
   *  dragging still fine-tunes the other one without relocating it. */
...
export function CourseMapPicker({
  ...
  useEffect(() => {
    ...
    // onMarkerMove/clickToPlaceId are read via the latest-ref pattern above
    // (onMarkerMoveRef/clickToPlaceIdRef), not closed over directly here, so
    // they deliberately aren't in this array — only `markers` should re-run
    // the marker sync.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [markers])
