...
export function HolesEditor({
  ...
  // Re-seed when the `holes` prop itself changes — e.g. "Re-fetch from OSM"
  // invalidates ['holes', courseId] (same courseId, new data), which the
  // courseId-keyed effect above never sees. Skipped while there are unsaved
  // base edits so an in-progress edit isn't silently discarded.
  useEffect(() => {
    if (baseDirty) return
    setRows(holes.map(toEditable))
    // eslint-disable-next-line react-hooks/exhaustive-deps
