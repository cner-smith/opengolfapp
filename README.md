...
- **Course database** — ~20,700 golf courses across the US, England,
  Scotland, Ireland, Wales and Northern Ireland, with GPS coordinates and
  per-tee rating/slope; fuzzy search backed by `pg_trgm`. Courses you add
  yourself are visible to you immediately and to everyone once reviewed.
...
- **Penalties** — out-of-bounds and stroke-and-distance marked on the live
  round, carried through the scorecard, shot lists and strokes gained.
- **Sign-in options** — email, Google, or Apple.
- **Round import** — bring rounds in from a JSON payload, provider-agnostic.

## Tech stack

- **Monorepo:** Turborepo + pnpm workspaces.
- **Web:** Vite + React 18 + TypeScript + Tailwind.
- **Mobile:** Expo SDK 54 + Expo Router + React Native 0.81 + React 19.1,
  New Architecture enabled. Styling is plain React Native `StyleSheet`.
  Builds run on EAS.
